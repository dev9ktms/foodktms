...
async def get_internal_mess_menu(institute:str,db:Session=Depends(get_db)):
    ...
    query1="SELECT * FROM vendor WHERE isApproved={}".format(True)+" AND status={}".format(True)
...
async def get_calories_history(email:str, date:str,db:Session=Depends(get_db)):
    ...
    deliveredOrders=cur.execute(query1).fetchall()
    data=[]
    ...
    cancelledOrders=cur.execute(query2).fetchall()
